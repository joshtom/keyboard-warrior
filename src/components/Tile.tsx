...
export function Tile({
  ...
  return (
    <button
      aria-label={isTouchMode ? "Tap tile" : `Hit ${tile.value}`}
    ...
    >
      {isTouchMode ? (
        <span
          aria-hidden="true"
          className="h-10 w-10 rounded-(--radius) border border-(--color-border-subtle) bg-(--color-accent-muted)"
        />
      ) : (
        <>
          <span
            aria-hidden="true"
            className="h-10 w-10 rounded-(--radius) border border-(--color-border-subtle) bg-(--color-accent-muted) sm:hidden"
          />
